...
export default function EnergyChart() {
  ...
  return <figure className="energy-research">
    <figcaption><div><span className="energy-kicker">THE EVIDENCE / IEA 2026</span><h4>Demand is scaling. Infrastructure takes time.</h4></div><label className="energy-select">Chart view<select value={view} onChange={event => setView(event.target.value as 'demand' | 'growth')}><option value="demand">Electricity demand</option><option value="growth">Relative growth</option></select></label></figcaption>
    <div className="energy-metrics"><div><strong>485 → 950 <small>TWh</small></strong><span>2025 estimate → 2030 projection</span></div><div><strong>~3×</strong><span>AI-focused consumption by 2030</span></div><div><strong>~3%</strong><span>Data centres’ projected share of global electricity in 2030</span></div></div>
    ...
    <div className="energy-context"><div><span>THE CONSTRAINT</span><p>Grid connections and equipment supply can limit how quickly new demand is served.</p></div><div><span>THE COUNTERWEIGHT</span><p>Efficiency is improving, while adoption and more intensive tasks can push consumption upward.</p></div><div><span>MY TAKE</span><p>Reliable access to power could become a competitive advantage for AI infrastructure.</p></div></div>
